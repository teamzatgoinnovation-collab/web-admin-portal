...
/** ERPNext username/password login via Next.js BFF (avoids CORS). */
export async function loginWithPassword(input: {
  ...
  const body = (await res.json().catch(() => ({}))) as LoginResult & {
    message?: string;
  };
  ...
  if (!res.ok || !body.ok) {
    const message =
      ("message" in body && typeof body.message === "string"
        ? body.message
        : null) || `Login failed (HTTP ${res.status})`;
...
/** Restore client state from BFF session cookie. */
...
export async function erpnextApi<T = unknown>(
  ...
  const json = JSON.parse(envelope.bodyText || "{}") as {
    message?: T;
  };
  if (!envelope.ok) {
    throw new Error(
